...
class PaymentConfigController extends AdminController
{
    ...
    public function form(): Form
    {
        return $this->baseForm(false)
            ...
            ->mode('horizontal')
            ->body([
                amis()->Switch('payment.wechat_pay.enable', '启用微信支付')->value(0),
                amis()->Switch('payment.alipay.enable', '启用支付宝支付')->value(0),
    ...
    private function wechatPayTab(): Tab
    {
        return amis()->Tab()->title('微信支付')->body([
            ...
            // V3 配置
            amis()->Group()->label('V3参数配置')->body([
                amis()->InputText('payment.wechat_pay.v3.mch_id', 'Mchid')
                    ->description('微信商户商户号'),
                amis()->InputText('payment.wechat_pay.v3.serial_no', '证书序列号')
                    ->description('「商户API证书」的「证书序列号」，可以在证书管理里面查看'),
                amis()->InputText('payment.wechat_pay.v3.private_key', 'V3支付Key')
                    ->description('V3支付秘钥'),
                amis()->InputText('payment.wechat_pay.v3.cert_path', '微信支付证书')
                    ->description('微信支付证书，在微信商家平台中可以下载！文件名一般为apiclient_cert.pem'),
                amis()->InputText('payment.wechat_pay.v3.key_path', '微信支付证书密钥')
                    ->description('微信支付证书密钥，在微信商家平台中可以下载！文件名一般为apiclient_key.pem'),
                amis()->Select('payment.wechat_pay.v3.mini_app_mode', '小程序商户号选择')
                    ->options([
                        ['label' => '商户号绑定', 'value' => 'merchant'],
                        ['label' => '非商户号绑定', 'value' => 'non_merchant']
                    ])
                    ->description('小程序开发后台有支付管理的请选择非商户号绑定'),
                amis()->Select('payment.wechat_pay.v3.merchant_type', '商户类型')
                    ->options([
            ...
            // V2 配置
            amis()->Group()->label('V2参数配置')->body([
                amis()->InputText('payment.wechat_pay.v2.key', 'Key')
                    ->description('商户支付密钥Key。审核通过后，在微信发送的邮件中查看。'),
                amis()->InputText('payment.wechat_pay.v2.mch_id', 'Mchid')
                    ->description('微信商户商户号'),
                amis()->InputText('payment.wechat_pay.v2.cert_path', '微信支付证书')
                    ->description('微信支付证书，在微信商家平台中可以下载！文件名一般为apiclient_cert.pem'),
                amis()->InputText('payment.wechat_pay.v2.key_path', '微信支付证书密钥')
                    ->description('微信支付证书密钥，在微信商家平台中可以下载！文件名一般为apiclient_key.pem'),
                amis()->Select('payment.wechat_pay.v2.mini_app_mode', '小程序商户号选择')
                    ->options([
                        ['label' => '商户号绑定', 'value' => 'merchant'],
                        ['label' => '非商户号绑定', 'value' => 'non_merchant']
                    ])
                    ->description('小程序开发后台有支付管理的请选择非商户号绑定'),
                amis()->Select('payment.wechat_pay.v2.merchant_type', '商户类型')
                    ->options([
                        ['label' => '普通微信商户模式', 'value' => 'normal'],
                        ['label' => '普通微信服务商模式', 'value' => 'service']
                    ])
    ...
    private function alipayTab(): Tab
    {
        return amis()->Tab()->title('支付宝')->body([
            ...
            amis()->InputText('payment.alipay.private_key', '支付应用私钥'),
        ]);
    }
