 ...
 * 微信 API 端点管理类
 * 
 * 统一管理所有微信 API 端点，方便更新和维护
 */
class WechatApiEndpoints
{
    /**
     * 公众号 API 端点
     */
    ...
    {
        if (!isset(self::OFFICIAL_ACCOUNT[$key])) {
            throw new \InvalidArgumentException("公众号 API 端点 '{$key}' 不存在");
        }
        return self::OFFICIAL_ACCOUNT[$key];
    }
    ...
    {
        if (!isset(self::MINI_PROGRAM[$key])) {
            throw new \InvalidArgumentException("小程序 API 端点 '{$key}' 不存在");
        }
        return self::MINI_PROGRAM[$key];
    }
