...
namespace warm\controller\dev_tools;
...
use warm\service\AdminMenuService;
use warm\support\code_generator\FilterGenerator;
use warm\support\code_generator\GenCodeClear;
use warm\support\code_generator\Generator;
use warm\trait\IconifyPickerTrait;
...
class CodeGeneratorController extends AdminController
{
    ...
    public function getPropertyOptions(Request $request): Response
    {
        ...
        $className = '\\jizhi\\admin\\renderer\\' . $request->input('c');
    ...
    public function saveComponentProperty(Request $request): Response
    {
        ...
        if ($original = settings()->get($request->input('key'))) {
        ...
        $res = settings()->set($request->input('key'), array_values($list));
    ...
    {
        $component_property_list = collect(settings()->get($request->input('key')))->values();
    ...
    {
        $list = settings()->get($request->input('name'));
        ...
        settings()->set($request->input('name'), array_values($list));
